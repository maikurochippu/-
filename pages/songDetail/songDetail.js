...
Page({
  ...
  async musicControl(isPlay,musicId){
    ...
    if(isPlay){//音乐播放
      ...
      if(musicLinkData.data[0].freeTrialInfo){
        wx.showToast({
          title: '由于版权或会员问题只能播放30秒',
          icon: 'none'
        })
        let durationTime = moment(30000).format('mm:ss');
        this.setData({
          durationTime
        })
      }else if(!musicLinkData.data[0].url){
        wx.showToast({
          title: '由于版权或会员问题无法播放',
          icon: 'none'
        })
      ...
      this.backgroundAudioManager.singer = this.data.song.ar.name;
